...
class DelegatingTableDao(
    ...
    override suspend fun getReserved() = active().getReserved()
    override suspend fun upsert(vararg tables: com.extrotarget.extropos.data.local.entity.TableEntity) = active().upsert(*tables)
    override suspend fun upsert(tables: List<com.extrotarget.extropos.data.local.entity.TableEntity>) = active().upsert(tables)
    ...
    override suspend fun clearOrder(tableId: String) = active().clearOrder(tableId)
    override suspend fun delete(tableId: String) = active().delete(tableId)
    override suspend fun delete(table: com.extrotarget.extropos.data.local.entity.TableEntity) = active().delete(table)
    override suspend fun getBySection(sectionId: String) = active().getBySection(sectionId)
    override suspend fun getByType(tableType: String) = active().getByType(tableType)
    override suspend fun getByCapacityRange(minCapacity: Int, maxCapacity: Int) = active().getByCapacityRange(minCapacity, maxCapacity)
    override suspend fun getBySmokingPreference(smokingAllowed: Boolean) = active().getBySmokingPreference(smokingAllowed)
    override suspend fun getAccessibleTables() = active().getAccessibleTables()
    override suspend fun getTablesWithPowerOutlets() = active().getTablesWithPowerOutlets()
    override suspend fun getByMinimumPriority(minPriority: Int) = active().getByMinimumPriority(minPriority)
    override suspend fun getAllSections() = active().getAllSections()
    override suspend fun getAllTableTypes() = active().getAllTableTypes()
    override suspend fun findAvailableTablesForTimeSlot(startTime: Long, endTime: Long, partySize: Int) = active().findAvailableTablesForTimeSlot(startTime, endTime, partySize)
    override suspend fun updateSectionActiveStatus(sectionId: String, isActive: Boolean, updatedAt: Long) = active().updateSectionActiveStatus(sectionId, isActive, updatedAt)
    override suspend fun getActiveTableCount() = active().getActiveTableCount()
    override suspend fun getTableCountByStatus(status: String) = active().getTableCountByStatus(status)
}
